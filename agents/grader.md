...
## Step 0：脚本预验证（所有 skill_type 通用，AI 评审前执行）

在进入任何 AI 评审步骤之前，先对 `exact_match` 类断言尝试脚本化验证，结果确定性 0/1，无需 AI 判断。
...
**这些断言在后续 AI 评审中跳过**，AI 只处理：
- `semantic` 类断言
- `existence` 类断言
- 无法映射到脚本类型的 `exact_match` 断言

> **为什么安全**：`tool_call_count`、`args_field`、`response_not_contains` 等类型的答案完全由 transcript 原文决定，正则/字符串匹配结果唯一确定，不存在主观空间。脚本验证通过率标注 `method: script`，报告中单独统计，与 `method: grader` 明确区分，提升结果可信度。
