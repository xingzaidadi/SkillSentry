...
- 新增 `scripts/sentry_timing.py`: 独立分析 `eval_result.json` 或 session timing artifacts,输出最慢 step/phase、executor per-eval 耗时分布、已有 grader per-eval 耗时分布和优化建议;`no-llm, no-network`。
