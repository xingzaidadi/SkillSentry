...
- **v5.0 新增**：增加「纯文本评审标准」章节，支持 text_generation Skill 的 evidence 从 response.md 引用；增加 `skill_type` 输入字段；增加效率指标（timing.json）读取
...
**v5.0 扩展**：grader.md 现在会主动读取 timing.json 并将 `executor_duration_ms` 和 `total_tokens` 写入 grading.json；generate_html_report.py 新增 `_render_efficiency_section` 函数聚合显示 P50/P95 响应时间。
...
|-------------------|---------|------|
| **触发率测评**（`run_eval.py`）| ⚠️ **部分集成（v5.0 新增）** | 真实测量仍需 claude CLI；已添加 AI 模拟替代方案（阶段一），产出置信度估算值，在报告第十一章展示 |
...
| **纯文本 Skill 测评** | ✅ **v5.0 新增** | Skill 类型自动检测（mcp_based/text_generation/code_execution），纯文本模式使用 response.md 作为 evidence 来源 |
| **效率层指标采集** | ✅ **v5.0 新增** | grader.md 读取 timing.json，generate_html_report.py 聚合展示 P50/P95/Token |
...
*本文档记录工具设计的技术溯源，供工程师参考。v5.0 更新：2026-03-23*
