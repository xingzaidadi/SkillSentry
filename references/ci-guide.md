...
完整 CI 运行会在 `session.json.ci_step_timings` 记录每个 pipeline step 的耗时,并在 `eval_result.json.timings`、`diagnostics.timings` 和 `summary.md` 中展示;这些字段只做观测,不改变 pipeline 步骤、gate 或退出码。
