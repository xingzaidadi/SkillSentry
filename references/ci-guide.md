...
可用 `python scripts/sentry_timing.py --input <eval_result.json|session_dir>` 单独分析最慢 step/phase;当能定位到 session 时,还会读取 `executor_results.json` 汇总 executor per-eval avg/p50/p95/max 和最慢用例。该工具只读 artifact,不调 LLM、不联网。
