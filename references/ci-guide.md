...
`local` 会写 `manifest.json`;复用同一 session 且输入 hash 未变时,会复用已准备的 `evals.json`/case lint 摘要并跳过 executor/grader。payload 会在 `executor.reuse.reason` / `grader.reuse.reason` 说明 `matched`、`input_hash_changed`、`missing_outputs` 或 force 等原因。需要重跑重步骤时使用 `--force-executor` 或 `--force-grader`。
...
可用 `python scripts/sentry_timing.py --input <eval_result.json|sentry-run-result.json|session_dir>` 单独分析最慢 step/phase;传入 local session 目录时会自动读取其中的 `sentry-run-result.json`。当能定位到 session 时,还会读取 `executor_results.json` 汇总 executor per-eval avg/p50/p95/max 和最慢用例,读取 `grading.json` 的 `duration_ms` 汇总 grader per-eval 耗时,并从 local profile payload 展示复用命中/未命中原因。该工具只读 artifact,不调 LLM、不联网。
---
...
| 当前文件 | `SkillSentry_repo\references\ci-guide.md` |
| 学习重点 | ci-guide |
