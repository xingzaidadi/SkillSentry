...
在 `lint` 阶段，除了看行数、重复和硬性规则，还可以顺手看“结构有没有开始难维护”。

**常见信号**:
- 重复流程太多：同样的判断、同样的步骤在多个地方复制
- 分支太密：一个入口里堆了很多 `if/else`、场景切换、例外分支
- 职责混在一起：一个 Skill 既管规则、又管示例、又管发布、又管通知
- 受众不同：新手、专家、运维、业务方的内容混在一份里
- 更新节奏不同：一部分内容经常改，一部分内容很稳定
- 所有权不同：不同团队负责不同段落，但放在同一个文件里

**lint 的建议输出**:
- 轻微：提示“这里有设计味道，建议观察”
- 中度：提示“建议拆分/抽象，不然后续维护会越来越难”
- 严重：直接标为高复杂度，纳入准入风险

**一句话记法**:
→ `lint` 不只看“对不对”，也看“会不会越写越乱”。
...
| 当前文件 | `SkillSentry_repo\references\pitfall-guide.md` |
| 学习重点 | pitfall-guide |
