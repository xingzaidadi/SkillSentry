...
- 每个 subagent 执行结束时写入 `timing_with.json` / `timing_without.json`：`executor_start_ms`、`executor_end_ms`、`duration_ms`
- Grader 读取 `timing_with.json`，将数据写入 `grading.json` 的 `timing` 字段
- `sentry-report` 从所有 `timing_with.json` / `timing_without.json` 聚合计算 P50/P95，在报告第八章展示

```json
// timing_with.json / timing_without.json 格式
{
...
**报告展示**：平均 Token 消耗/用例，在报告第八章「效率指标」与响应时间一起展示。
...
| 当前文件 | `SkillSentry_repo\references\archive\eval-dimensions-full.md` |
| 学习重点 | eval-dimensions-full |
