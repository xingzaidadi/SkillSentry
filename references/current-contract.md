...
- `sentry_ci.py` 必须在 `session.json.ci_step_timings` / `ci_timing` 记录 pipeline step 耗时,并在 `eval_result.json.timings` 和 diagnostics 中透出;timings 不得改变 pipeline 顺序、gate 或退出码。
