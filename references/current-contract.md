...
| `scripts/sentry_timing.py` | 读取 `eval_result.json` 或 session timing artifacts,排序慢 step/phase,汇总 executor per-eval 耗时并给出观测建议 | 不调 LLM、不联网、不改变 pipeline/gate/退出码 |
