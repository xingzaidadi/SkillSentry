...
**本文件是 SkillSentry v7.7.0，是唯一权威的行为定义。**
...
- **⛔ 所有模式必须 spawn sentry-cases subagent**：禁止主会话自己编写 evals.json，即使是 smoke 模式也必须走 sentry-cases 完整流程（需求分析 → 数据采集 → 用例设计 → 用户确认）
- **⛔ grader 完成后必须产出三件套**：eval-report.html + grading-summary.json + history.json。HTML 报告必须上传飞书并发送下载链接给用户。
...
*v7.7.0 · security用例+C1C2C4C5E1指标+强制spawn sentry-cases+三件套产出 · 2026-04-29*
