...
执行前必须读取以下文件，不读不得开始：
1. 被测 SKILL.md 全文
2. inputs_dir/rules.cache.json（如存在）
3. inputs_dir/requirements.cache.json（如存在）
...
自动创建 `~/.openclaw/data/skill-eval/sessions/<Skill名>/<YYYY-MM-DD>_<NNN>/`。
...
- anti_laziness: 反偷懒测试（full 模式 ≥2 个，测试是否依赖旧记忆/跳步/假完成）
...
| 当前文件 | `SkillSentry_repo\tools\sentry-cases\SKILL.md` |
| 学习重点 | SKILL |
