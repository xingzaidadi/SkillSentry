---
name: sentry-comparator
description: >
  盲测对比 Agent，独立评判 with_skill vs without_skill 输出质量优劣。
  仅在 standard/full 模式中由主编排调用。
---
...
执行前必须读取以下文件，不读不得开始：
1. 每个 eval 的 with_skill/outputs/response.md
2. 每个 eval 的 without_skill/outputs/response.md
...
以下示例展示 **B 胜出** 的完整输出格式。A 不一定总是赢：

```json
{
  "winner": "B",
  "reasoning": "B 严格按照输出模板格式化了查询结果，包含审批状态、金额明细和可用链接。A 虽然调用了正确的 MCP 工具，但直接输出了原始 JSON 而未做格式化，用户无法直接阅读。",
  "rubric": {
    "A": {
      "content": { "correctness": 4, "completeness": 3, "accuracy": 4 },
      "structure": { "organization": 2, "formatting": 1, "usability": 2 },
      "content_score": 3.7,
      "structure_score": 1.7,
      "overall_score": 5.4
    },
...
> **评判员须知**：with_skill 不一定比 without_skill 好。如果 Skill 的规则写得差（如遗漏格式化步骤），without_skill 的自由发挥可能反而更好。这是重要的测评发现，不是异常。
---
...
| 当前文件 | `SkillSentry_repo\tools\sentry-comparator\SKILL.md` |
| 学习重点 | SKILL |
