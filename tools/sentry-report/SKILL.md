...
执行前必须读取以下文件，不读不得开始：
1. workspace_dir/grading-summary.json
2. inputs_dir/history.json（如存在）
...
**飞书文档输出（standard/full 模式，优先级高于 HTML）**：

除 HTML 归档外，standard/full 模式必须额外生成飞书在线文档：
1. 使用 feishu_create_doc 创建文档，标题格式：`SkillSentry · {skill_name} · {date} · {grade}级`
2. 内容为 Markdown 格式的完整报告（同 HTML 内容，但用 Markdown 标记）
3. 文档所有权转让给用户
4. 在最终消息中发送文档链接

HTML 定位为本地归档，飞书文档定位为在线查看。
...
```bash
python3 {skill-eval-测评根目录}/scripts/update_history.py \
  --skill <skill_name> \
...
|---|---|
| 所属主题 | 报告输出与模板交付 |
| 当前文件 | `SkillSentry_repo\tools\sentry-report\SKILL.md` |
| 学习重点 | SKILL |
