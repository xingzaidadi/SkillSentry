...
runTimeoutSeconds = 900（15min，足够 30 eval × 1 run，含重试余量）
```
...
| 用例 | 用户输入 | 路由决策 | 执行结果 |
|------|---------|---------|----------|
| hp_01 | "查BR202503250232" | BR前缀→budget MCP | 返回详情：已撤回 ¥3.00 |
| hp_02 | "查E单号" | E前缀→expense MCP | 查无结果，提示不存在 |

**edge_case（{M}）**
| 用例 | 用户输入 | 路由决策 | 执行结果 |
|------|---------|---------|----------|
| ec_01 | "查我的单据" | 未指定类型→不调MCP | 引导选择四种类型 |

**negative（{M}）**
| 用例 | 用户输入 | 路由决策 | 执行结果 |
|------|---------|---------|----------|
| neg_01 | "12345678" | 无合法前缀→拦截 | 提示格式不合法 |

**robustness（{M}）**
| 用例 | 用户输入 | 路由决策 | 执行结果 |
|------|---------|---------|----------|
| rob_01 | "BR+空格" | 去空格→budget | 正常查询成功 |
...
| 当前文件 | `SkillSentry_repo\tools\sentry-executor\SKILL.md` |
| 学习重点 | SKILL |
