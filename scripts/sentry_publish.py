...
def ensure_report(session_dir: Path, gate: dict) -> Path:
    ...
    if not should_write_minimal_report(report):
        return report
    rate = gate.get("authoritative_pass_rate")
...
<body>
  <h1>SkillSentry Publish Result</h1>
  <p><strong>Verdict:</strong> {gate.get("verdict", "UNKNOWN")}</p>
  <p><strong>Grade:</strong> {gate.get("grade", "N/A")}</p>
  <p><strong>Authoritative pass rate:</strong> {rate_text}</p>
  <p><strong>Delta:</strong> {gate.get("delta", {}).get("status", "N/A")}</p>
{diagnostics_html}
  <p>Generated by <code>sentry_publish.py</code>. Interactive publishing can still use <code>publish.py</code>.</p>
</body>
</html>
"""
