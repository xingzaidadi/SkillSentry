...
def make_duplicate_cases() -> list[dict]:
    return [
        {"id": "eval-1", "prompt": "fast"},
        {"id": "eval-1", "prompt": "duplicate"},
        {"id": "eval-2", "prompt": "slow"},
    ]
...
def verify_session(root: Path, errors: list[str]) -> None:
    ...
    duplicate_dir = root / "duplicate-case-session"
    duplicate_dir.mkdir()
    sentry_state.save_session(duplicate_dir, {"skill": "timing-fixture", "mode": "local"})
    save_json(duplicate_dir / "evals.json", make_duplicate_cases())
    save_json(duplicate_dir / "executor_results.json", make_executor_payload())
    save_json(duplicate_dir / "eval-1" / "grading.json", make_grading_payload("eval-1", 300.0))
    save_json(duplicate_dir / "eval-2" / "grading.json", make_grading_payload("eval-2", 900.0))
    duplicate_payload = sentry_timing.analyze(duplicate_dir, top=5)
    if duplicate_payload.get("executor_timing", {}).get("expected_cases") != 2:
    ...
    save_json(case_id_dir / "evals.json", make_case_id_cases())
    save_json(case_id_dir / "executor_results.json", {"results": [{"eval_id": "eval-1", "status": "success", "duration": 0.2}]})
    save_json(case_id_dir / "eval-1" / "grading.json", make_grading_payload("eval-1", 300.0))
    case_id_payload = sentry_timing.analyze(case_id_dir, top=5)
    if case_id_payload.get("executor_timing", {}).get("expected_cases") != 2:
        errors.append("case_id-only cases should fall back to eval-N executor artifact ids")
    if case_id_payload.get("grader_timing", {}).get("missing_case_ids") != ["eval-2"]:
        errors.append("case_id-only cases should report missing eval-N grader artifact ids")
