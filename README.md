...
| `scripts/sentry_timing.py` | 独立轻量耗时分析器,读取 `eval_result.json` 或 session,输出最慢 step/phase、executor per-eval 耗时分布、已有 grader per-eval 耗时分布和优化建议;不调 LLM、不联网。 |
